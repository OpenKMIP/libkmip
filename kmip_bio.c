...
int kmip_bio_create(BIO *bio, struct template_attribute *template_attribute,
...
int kmip_bio_destroy(BIO *bio, char *uuid, size_t uuid_size)
{
    /* Set up the KMIP context and the initial encoding buffer. */
    struct kmip ctx = {0};
    kmip_init(&ctx, NULL, 0, KMIP_1_0);
...
int kmip_bio_get_symmetric_key(
BIO *bio,
char *id, size_t id_size,
...
int kmip_bio_destroy_with_context(
struct kmip *ctx, BIO *bio,
