...
int
encode_name(struct kmip *ctx, struct name *value)
{
...
int
encode_attribute(struct kmip *ctx, struct attribute *value)
{
    ...
    switch(value->type)
    {
        ...
        result = encode_text_string(
            ctx, t, 
            ((struct text_string*)value->value)->value,
            ((struct text_string*)value->value)->size);
        break;
...
int
encode_template_attribute(struct kmip *ctx, struct template_attribute *value)
{
    int result = 0;
                         ...
                         const struct key_wrapping_data *value)
{
    ...
    if(value->mac_signature != NULL)
    {
        result = encode_byte_string(
            ...
            value->mac_signature->value,
            value->mac_signature->size);
        CHECK_RESULT(ctx, result);
    }
    ...
    if(value->iv_counter_nonce != NULL)
    {
        result = encode_byte_string(
            ...
            value->iv_counter_nonce->value,
            value->iv_counter_nonce->size);
        CHECK_RESULT(ctx, result);
    }
...
encode_key_material(struct kmip *ctx, enum key_format_type format, const void *value)
{
    ...
    switch(format)
    {
        ...
        result = encode_byte_string(
            ...
            ((struct byte_string*)value)->value,
            ((struct byte_string*)value)->size);
        CHECK_RESULT(ctx, result);
        return(KMIP_OK);
        break;
...
encode_key_block(struct kmip *ctx, const struct key_block *value)
{
    ...
    if(value->key_wrapping_data != NULL)
    {
        result = encode_byte_string(
            ctx,
            KMIP_TAG_KEY_VALUE,
            ((struct byte_string*)value->key_value)->value,
            ((struct byte_string*)value->key_value)->size);
    }
    else
    {
                                  ...
                                  const struct key_wrapping_specification *value)
{
    ...
    for(size_t i = 0; i < value->attribute_name_count; i++)
    {
        result = encode_text_string(
            ...
            value->attribute_names[i].value,
            value->attribute_names[i].size);
        CHECK_RESULT(ctx, result);
    }
    ...
    if(ctx->version >= KMIP_1_4)
    {
        if(value->client_correlation_value != NULL)
        {
            result = encode_text_string(
                ...
                value->client_correlation_value->value,
                value->client_correlation_value->size);
            CHECK_RESULT(ctx, result);
        }
        ...
        if(value->server_correlation_value != NULL)
        {
            result = encode_text_string(
                ...
                value->server_correlation_value->value,
                value->server_correlation_value->size);
            CHECK_RESULT(ctx, result);
        }
    }
